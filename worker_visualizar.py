        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                if notas:
                    ...
                    # ── Generate SCI-importable XMLs (SefinNacional v1.01, Layout 62) ──
                    log(name, "Gerando XMLs para importacao no SCI...")
                    try:
                        sci_result = save_reconstructed_xmls(notas, download_dir)
                        log(name, f"SCI com retencao: {len(sci_result['com_retencao'])} | sem retencao: {len(sci_result['sem_retencao'])}")
                    except Exception as e:
                        log(name, f"[AVISO] Falha ao gerar XMLs SCI: {e}")
