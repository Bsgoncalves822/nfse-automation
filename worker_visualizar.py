...
def scrape_all_pages(page, company_name):
    ...
    while True:
        ...
        proxima = page.query_selector("a[data-original-title='Proxima']") or \
                  page.query_selector("a[data-original-title='Pr\u00f3xima']")
        ultima  = page.query_selector("a[data-original-title='Ultima']") or \
                  page.query_selector("a[data-original-title='\u00daltima']")
        if not proxima and not ultima:
            break
        if pg >= 50:
            break
        pg += 1
        next_url = re.sub(r'pg=\d+', f'pg={pg}', base_url) if 'pg=' in base_url \
                   else base_url + ('&' if '?' in base_url else '?') + f'pg={pg}'
        page.goto(next_url)
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                ...
                log(name, f"{len(chaves)} nota(s) - raspando dados...")
                notas  = []
                failed = 0
                for i, chave in enumerate(chaves, 1):
                    log(name, f"[{i}/{len(chaves)}] {chave}")
                    for attempt in range(3):
                        try:
                            data = scrape_visualizar(page, chave)
                            if data: notas.append(data)
                            else: failed += 1
                            break
                        except SessionExpiredError:
                            log(name, f"Sessao expirou - re-login ({attempt+1}/3)")
                            try: page.close()
                            except: pass
                            page = login(context, cnpj, password, name)
                            if not page: break
                            if mode == 'emitidas': navigate_to_emitidas(page)
                            else: navigate_to_recebidas(page)
                            apply_filter(page, custom_start, custom_end)
                    time.sleep(0.5)
                ...
                log(name, f"{len(notas)} notas | {failed} falhas")

                if notas:
                    federal = [n for n in notas if n['is_federal'] and not n.get('is_cancelada')]
                    log(name, f"Federal: {len(federal)}")

                    excel_path = generate_visualizar_excel(name, month, notas, download_dir, company_cnpj=cnpj)
                    log(name, f"Excel: {excel_path}")

                    if federal:
                        xml_paths = save_reconstructed_xmls(notas, download_dir, federal_only=True)
                        log(name, f"{len(xml_paths)} XMLs reconstruidos")
                        try:
                            import importlib
                            import generate_fiscal as gf
                            importlib.reload(gf)
                            r = gf.generate_fiscal(name, download_dir, month)
                            if r: log(name, f"Fiscal XLSX: {r}")
                            t = gf.generate_fiscal_txt(name, download_dir, month)
                            if t: log(name, f"Fiscal TXT: {t}")
                        except Exception as e:
                            log(name, f"[AVISO] Fiscal: {e}")
