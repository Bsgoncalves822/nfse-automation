...
def run_zip():
    ...
    try:
        ...
        # Try to reuse company data from the temp config written by run/stream
        # to avoid a second Google Sheets fetch that might fail
        ...
        if os.path.exists(temp_file):
            try:
                ...
                if selected:
                    selected_companies = [c for c in candidates if c['cnpj'] in selected]
                else:
                    selected_companies = candidates
                print(f'[ZIP] Usando temp_run.json: {len(selected_companies)} empresa(s)', flush=True)
            except Exception as e:
                print(f'[ZIP] Falha ao ler temp_run.json: {e}', flush=True)
